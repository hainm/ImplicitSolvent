...
class Molecule:
    ...
    def get_atoms_from_top(self, top):
        ...
        # set atom types, charges, radii for atoms of molecule and add atoms to
        # array stored in molecule.atoms
    ...
    def get_atoms(self, top, crd):
      ...
      '''
        ...
        for i in range(1, numline + 1):
            ...
            try:
                self.atoms[
                    index + 1].set_coord(float(line[3]), float(line[4]), float(line[5]))
    ...
    def get_coord_from_PDB(self, pdb):
        ...
        if lines[0].startswith("CRYST1"):  # generated from vmd.
            try:
                ...
                # skip header "CRYST1" in PDB file generated by vmd, remove
                # "END" line
                arrpdb = loadtxt(
                    pdb, skiprows=1, comments="END", usecols=(6, 7, 8))
            except:  # re-write pdb file
                self.write_to_new_PDB(pdb)
                arrpdb = loadtxt(
                    pdb + ".mod", skiprows=1, comments="END", usecols=(6, 7, 8))
        elif lines[0].startswith("REMARK"):  # generated from ptraj
            #            print 'test'
            try:
                # no problem with coordinate (see Molecule.write_to_new_PDB())
                # skip header "REMARK" in PDB file generated by ptraj, remove
                # "END" line
                arrpdb = loadtxt(
                    pdb, skiprows=1, comments="END", usecols=(5, 6, 7))
#                print arrpdb.__len__()
            except:  # re-write pdb file
                self.write_to_new_PDB(pdb)
                arrpdb = loadtxt(
                    pdb + ".mod", skiprows=1, comments="END", usecols=(5, 6, 7))
        else:
            # no header: generated by ambpdb and then removing header
            # pdb file generated by tleap have coordinates in colum 6,7,8
            # (index: 5,6,7)
            try:
                # there is no problem with coordinate (see Molecule.write_to_new_PDB())
                # remove "END" line, do not skip first line. Make sure to
                # remove TER
                arrpdb = loadtxt(
                    pdb, skiprows=0, comments="END", usecols=(5, 6, 7))
            except:  # re-write pdb file
                self.write_to_new_PDB(pdb)
                arrpdb = loadtxt(
                    pdb + ".mod", skiprows=0, comments="END", usecols=(5, 6, 7))
    ...
    def get_solv(self):
        ...
        for i in range(self.natoms):
            ...
            if atomi.R == 0:
                ...
                sumSelf = sumSelf + sq(atomi.q) / atomi.R
        ...
        # 1.0 and 0.5 are BIG different. If you use 1.0 here, you would get the
        # correct answers.
        sumCross = -1.0 * (1 / self.ein - 1 / self.eout) * sumCross
    ...
    def get_solvFromFortranFull(self, igb, egbVersion):
        ...
        if igb == 5:
            scalingDict = {
                'H': 0.85, 'C': 0.72, 'N': 0.79, 'O': 0.85, 'S': 0.96}
    ...
    def getSelfEne_NCHO(self, gbArray_NCHO, egbVersion):
        ...
        egb = egbVersion.effrad.egb_calc_radii
        reff = egb(self.xx, self.fss, FSMAX, RGBMAX, self.rborn,
                   offset, gbvalpha, gbvbeta, gbvgamma)[0]
        self.getEffRadFromReff(reff)
        tmp = -0.5 * (1 / self.ein - 1 / self.eout)
        for i in range(len(self.atoms)):
            atomi = self.atoms[i]
    ...
    def get_back_bone_based_arr(self, reff):
        ...
        for i in range(N):
            if i == N - 1:
                ...
                ind_1, ind_2 = self.respt[i], self.respt[i + 1]
            res.append([reff[ind] for ind in range(
                ind_1, ind_2) if self.is_backbone(self.atname_pdb[ind])])
        return res
    ...
    def get_back_bone_based_arr_addH(self, reff):
        ...
        for i in range(N):
            if i == N - 1:
                ...
                ind_1, ind_2 = self.respt[i], self.respt[i + 1]
            res.append([reff[ind] for ind in range(ind_1, ind_2)
                        if self.is_backbone_addH(self.atname_pdb[ind])])
        return res
...
def test_gbEne(top, pdb, igb, egbVersion, sf=''):
    ...
    if sf:
        scalingDict = {
            'H': sf[0], 'C': sf[1], 'N': sf[2], 'O': sf[3], 'S': sf[4]}
        mol.offset = sf[5]
